...
size_t N = 10; // set the timestep length to be large enough to predict further but not too much
...
double ref_cte = 0;
double ref_epsi = 0;
double ref_v = 40; // set the reference velocity
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N; i++) {
      fg[0] += 600*CppAD::pow(vars[cte_start + i] - ref_cte, 2);
    ...
    for (int i = 0; i < N - 1; i++) {
      fg[0] += 9600*CppAD::pow(vars[delta_start + i], 2);
    ...
    for (int i = 0; i < N - 2; i++) {
      ...
      fg[0] += 9000*CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
